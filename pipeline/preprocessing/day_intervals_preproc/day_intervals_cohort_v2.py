...
def extract_data(use_ICU:str, label:str, time:int, icd_code:str, root_dir, disease_label, cohort_output=None, summary_output=None, data_dir = None, ver='2.0/'):
    ...
    if(data_dir == None):
        data_dir = root_dir+"/mimiciv/"+ver
    else:
        data_dir = data_dir+"/mimiciv/"+ver

    pts = get_visit_pts(
        mimic4_path=data_dir,
        group_col=group_col,
        visit_col=visit_col,
        admit_col=admit_col,
        disch_col=disch_col,
    ...
    if use_disease:
        hids=disease_cohort.extract_diag_cohort(cohort['hadm_id'],icd_code,data_dir)
        #print(hids.shape)
        #print(cohort.shape)
