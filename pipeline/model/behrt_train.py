...
class train_behrt():
    def __init__(self,src, age, sex, ethni, ins, target_data):
        ...
        self.conf = BertConfig(self.model_config)
        self.behrt = BertForEHRPrediction(self.conf, self.model_config['number_output'])
        
        self.behrt = self.behrt.to(self.train_params['device'])

        #models parameters
        self.transformer_vars = [i for i in self.behrt.parameters()]

        #optimizer
        self.optim_behrt = torch.optim.Adam(self.transformer_vars, lr=3e-5)
