...
class Loss(nn.Module):
    def __init__(self,name,device,acc,ppv,sensi,tnr,npv,auroc,aurocPlot,auprc,auprcPlot,callb,callbPlot):
...
#             print(prob)
    ...
    def auroc_plot(self,label, pred):
        ...
        plt.plot(fpr, tpr, label=f'{self.name}, auc = {str(round(auc,3))}')


        plt.ylabel("True Positive Rate")
        plt.xlabel("False Positive Rate")
        plt.title("AUC-ROC for {}".format(self.name))
        plt.legend()
    ...
    def calb_curve(self,bins,bin_accs,ECE, MCE):
        ...
        plt.title('Calibration curve for {}'.format(self.name))
